...
const QODERWORK_FLOW_NOTICE_COLLAPSED_KEY = 'agtools.qoderwork.flow_notice_collapsed';
const QODERWORK_FILTER_PERSISTENCE_SCOPE = normalizeAccountsOverviewScope('qoderwork_intl');
const QODERWORK_FILTER_FIELD_VIEW_MODE = 'view_mode';
const QODERWORK_FILTER_FIELD_SORT_BY = 'sort_by';
const QODERWORK_FILTER_FIELD_SORT_DIRECTION = 'sort_direction';
const QODERWORK_FILTER_FIELD_FILTER_TYPES = 'filter_types';
const QODERWORK_FILTER_FIELD_TAG_FILTER = 'tag_filter';
const QODERWORK_FILTER_FIELD_GROUP_BY_TAG = 'group_by_tag';
...
function logQoderWorkOauthUi(stage: string, payload?: Record<string, unknown>) {
  if (payload) {
    console.info('[QoderWork OAuth UI]', stage, payload);
    return;
  }
    console.info('[QoderWork OAuth UI]', stage);
}

const QODERWORK_OAUTH_START_TIMEOUT_ERROR = 'QODERWORK_OAUTH_START_TIMEOUT';
const QODERWORK_OAUTH_START_TIMEOUT_MS = 10000;
const QODERWORK_OAUTH_PEEK_TIMEOUT_ERROR = 'QODERWORK_OAUTH_PEEK_TIMEOUT';
const QODERWORK_OAUTH_PEEK_TIMEOUT_MS = 1200;
const QODERWORK_OAUTH_PEEK_RETRY_MAX = 8;
const QODERWORK_OAUTH_PEEK_RETRY_INTERVAL_MS = 250;
...
export function QoderWorkIntlAccountsPage() {
  const { t } = useTranslation();
  const store = useQoderWorkIntlAccountStore();
  const initialFilterPersistenceEnabled =
    readAccountsOverviewFilterPersistenceEnabled(QODERWORK_FILTER_PERSISTENCE_SCOPE);
    ...
    initialFilterPersistenceEnabled
      ? normalizeQoderViewMode(
          readAccountsOverviewFilterField<unknown>(
            QODERWORK_FILTER_PERSISTENCE_SCOPE,
            QODERWORK_FILTER_FIELD_VIEW_MODE,
            'grid',
          ),
        )
      : 'grid',
  ...
  const [filterTypes, setFilterTypes] = useState<string[]>(() =>
    initialFilterPersistenceEnabled
      ? readAccountsOverviewFilterStringArray(
          QODERWORK_FILTER_PERSISTENCE_SCOPE,
          QODERWORK_FILTER_FIELD_FILTER_TYPES,
        )
      : [],
  );
  const [sortBy, setSortBy] = useState<SortBy>(() =>
    initialFilterPersistenceEnabled
      ? normalizeQoderSortBy(
          readAccountsOverviewFilterField<unknown>(
            QODERWORK_FILTER_PERSISTENCE_SCOPE,
            QODERWORK_FILTER_FIELD_SORT_BY,
            'created_at',
          ),
        )
      : 'created_at',
  ...
  const [sortDirection, setSortDirection] = useState<SortDirection>(() =>
    initialFilterPersistenceEnabled
      ? normalizeQoderSortDirection(
          readAccountsOverviewFilterField<unknown>(
            QODERWORK_FILTER_PERSISTENCE_SCOPE,
            QODERWORK_FILTER_FIELD_SORT_DIRECTION,
            'desc',
          ),
        )
      : 'desc',
  ...
  const [tagFilter, setTagFilter] = useState<string[]>(() =>
    initialFilterPersistenceEnabled
      ? readAccountsOverviewFilterStringArray(
          QODERWORK_FILTER_PERSISTENCE_SCOPE,
          QODERWORK_FILTER_FIELD_TAG_FILTER,
        )
      : [],
  );
  ...
  const [groupByTag, setGroupByTag] = useState<boolean>(() =>
    initialFilterPersistenceEnabled
      ? Boolean(
          readAccountsOverviewFilterField<unknown>(
            QODERWORK_FILTER_PERSISTENCE_SCOPE,
            QODERWORK_FILTER_FIELD_GROUP_BY_TAG,
            false,
          ),
        )
      : false,
  ...
  const [isFlowNoticeCollapsed, setIsFlowNoticeCollapsed] = useState<boolean>(() =>
    readBooleanStorage(QODERWORK_FLOW_NOTICE_COLLAPSED_KEY, false),
  );
  ...
  useEffect(() => {
    const handleFilterPersistenceChanged = (event: Event) => {
      ...
      if (!detail || detail.scope !== QODERWORK_FILTER_PERSISTENCE_SCOPE) {
        return;
      }
  ...
  const exportModal = useExportJsonModal({
    exportFilePrefix: 'qoderwork_accounts',
    exportJsonByIds: qoderWorkIntlService.exportQoderWorkAccounts,
    onError: (error) =>
      setMessage({
        tone: 'error',
  ...
  /** 关闭弹窗时同步关闭应用内授权窗口，避免残留无痕会话。 */
  const closeAddModal = useCallback(() => {
    setShowAddModal(false);
    void qoderWorkIntlService.closeQoderWorkOAuthWindow().catch(() => {});
  }, []);
  ...
  const consumeExternalProviderImport = useCallback(() => {
    const request = consumeQueuedExternalProviderImportForPlatform('qoderwork_intl');
    if (!request) return;
    openAddModal('token');
  ...
  useEffect(() => {
    if (!showAddModal) {
      ...
      if (loginId) {
        logQoderWorkOauthUi('session:cancel-on-modal-close', { loginId });
        void qoderWorkIntlService.cancelQoderWorkOAuthLogin(loginId).catch(() => {});
      }
      setAddStatus('idle');
      setAddMessage(null);
      setAddTab('import');
  ...
  useEffect(() => {
    ...
    const loginId = oauthSessionRef.current ?? oauthLoginId ?? undefined;
    if (loginId) {
      logQoderWorkOauthUi('session:cancel-on-tab-change', { loginId, addTab });
      void qoderWorkIntlService.cancelQoderWorkOAuthLogin(loginId).catch(() => {});
    }
    resetOauthState();
  ...
  useEffect(
    () => () => {
      ...
      if (loginId) {
        logQoderWorkOauthUi('session:cancel-on-unmount', { loginId });
        void qoderWorkIntlService.cancelQoderWorkOAuthLogin(loginId).catch(() => {});
      }
      oauthSessionRef.current = null;
      oauthCompletingLoginIdRef.current = null;
  ...
  /** QoderWork 不是 VS Code 分支，没有 state.vscdb 可导入；提供明确错误而不是静默失败。 */
  const handleImportLocal = useCallback(async () => {
    setAddStatus('error');
    setAddMessage('QoderWork 暂不支持从 Qoder CLI 本机导入');
  }, []);
  ...
  const handlePrepareOauth = useCallback(async () => {
    ...
    logQoderWorkOauthUi('prepare:start', {
      oauthPreparing,
      oauthCompleting,
    ...
    if (previousLoginId) {
      logQoderWorkOauthUi('prepare:cancel-previous', { loginId: previousLoginId });
      await qoderWorkIntlService.cancelQoderWorkOAuthLogin(previousLoginId).catch(() => {});
    }
    ...
    try {
      const startCompletePolling = (loginId: string) => {
        ...
        void qoderWorkIntlService
          .completeQoderWorkOAuthLogin(loginId)
          .then(async (completedAccount) => {
            logQoderWorkOauthUi('complete:resolved', { loginId });
            ...
            for (let attempt = 0; attempt < 3; attempt += 1) {
              await store.fetchAccounts();
              const state = useQoderWorkIntlAccountStore.getState();
            ...
            if (!accountVisible) {
              logQoderWorkOauthUi('complete:account-not-visible-after-refresh', {
                loginId,
          ...
          .catch((error) => {
            logQoderWorkOauthUi('complete:rejected', {
              loginId,
              error: String(error),
            });
            if (attemptSeq !== oauthAttemptSeqRef.current) return;
      ...
      let response: qoderWorkIntlService.QoderWorkOAuthStartResponse;
      try {
        logQoderWorkOauthUi('prepare:invoke-start', { timeoutMs: QODERWORK_OAUTH_START_TIMEOUT_MS });
        response = await withTimeout(
          qoderWorkIntlService.startQoderWorkOAuthLogin(),
          QODERWORK_OAUTH_START_TIMEOUT_MS,
          QODERWORK_OAUTH_START_TIMEOUT_ERROR,
        );
        if (attemptSeq !== oauthAttemptSeqRef.current) {
          void qoderWorkIntlService.cancelQoderWorkOAuthLogin(response.loginId).catch(() => {});
          logQoderWorkOauthUi('prepare:start-stale-after-start', {
            loginId: response.loginId,
            attemptSeq,
          });
          return;
        }
        logQoderWorkOauthUi('prepare:invoke-resolved', {
          loginId: response.loginId,
        ...
        const isTimeout = startErrorText.includes(QODERWORK_OAUTH_START_TIMEOUT_ERROR);
        if (!isTimeout) {
          throw startError;
        }

        logQoderWorkOauthUi('prepare:start-timeout', {
          timeoutMs: QODERWORK_OAUTH_START_TIMEOUT_MS,
          error: startErrorText,
        });

        let peeked: qoderWorkIntlService.QoderWorkOAuthStartResponse | null = null;
        for (let attempt = 1; attempt <= QODERWORK_OAUTH_PEEK_RETRY_MAX; attempt += 1) {
          if (attemptSeq !== oauthAttemptSeqRef.current) return;
          const pending = await withTimeout(
            qoderWorkIntlService.peekQoderWorkOAuthLogin(),
            QODERWORK_OAUTH_PEEK_TIMEOUT_MS,
            QODERWORK_OAUTH_PEEK_TIMEOUT_ERROR,
          ).catch((peekError) => {
            const errorText = String(peekError);
            logQoderWorkOauthUi('prepare:peek-error', {
              attempt,
              error: errorText,
              timeout: errorText.includes(QODERWORK_OAUTH_PEEK_TIMEOUT_ERROR),
            });
            return null;
          });
          if (attemptSeq !== oauthAttemptSeqRef.current) {
            if (pending?.loginId) {
              void qoderWorkIntlService.cancelQoderWorkOAuthLogin(pending.loginId).catch(() => {});
            }
            return;
          }
          if (pending?.loginId && pending?.verificationUri) {
            peeked = pending;
            logQoderWorkOauthUi('prepare:peek-hit', {
              attempt,
          ...
          await delay(QODERWORK_OAUTH_PEEK_RETRY_INTERVAL_MS);
        }
      ...
      if (attemptSeq !== oauthAttemptSeqRef.current) {
        void qoderWorkIntlService.cancelQoderWorkOAuthLogin(response.loginId).catch(() => {});
        logQoderWorkOauthUi('prepare:start-stale-before-apply', {
          loginId: response.loginId,
          attemptSeq,
        });
        return;
      ...
      if (!verificationUri) {
        ...
        logQoderWorkOauthUi('prepare:verification-uri-empty', {
          loginId,
      ...
      logQoderWorkOauthUi('prepare:will-set-state', {
        loginId,
      ...
      setOauthPreparing(false);
      logQoderWorkOauthUi('prepare:state-set-done', { loginId });
      startCompletePolling(loginId);
    } catch (error) {
      if (attemptSeq !== oauthAttemptSeqRef.current) return;
      logQoderWorkOauthUi('prepare:start-failed', { error: String(error) });
      const msg = String(error);
      setOauthPreparing(false);
      setOauthCompleting(false);
      setOauthError(msg);
  ...
  const handleOpenOauthUrl = useCallback(async () => {
    if (!oauthUrl) return;
    try {
      logQoderWorkOauthUi('link:open-incognito-window', { urlLength: oauthUrl.length });
      await qoderWorkIntlService.openQoderWorkOAuthWindow(oauthUrl);
    } catch (error) {
      logQoderWorkOauthUi('link:open-incognito-window-failed', { error: String(error) });
      const msg = String(error);
      setOauthError(msg);
      setAddStatus('error');
      setAddMessage(t('common.shared.oauth.failed', '授权失败') + ': ' + msg);
  ...
  useEffect(() => {
    ...
    logQoderWorkOauthUi('effect:auto-prepare-fire');
  ...
  useEffect(() => {
    ...
    const adoptPendingSession = async () => {
      const pending = await qoderWorkIntlService.peekQoderWorkOAuthLogin().catch(() => null);
      ...
      setOauthUrl(verificationUri);
      setOauthError(null);
      logQoderWorkOauthUi('prepare:adopt-pending-session', {
        loginId: pending.loginId,
        verificationUriLength: verificationUri.length,
      });
    };
            ...
            <button
              ...
              onClick={() =>
                setIsFlowNoticeCollapsed((prev) => {
                  const next = !prev;
                  writeBooleanStorage(QODERWORK_FLOW_NOTICE_COLLAPSED_KEY, next);
                  return next;
                })
              }
