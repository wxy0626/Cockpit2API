...
func main() {
	...
	// 日志双写：标准输出照旧，环形缓冲供管理端 /api/logs 读取（sidecar 模式下无 docker logs）
	log.SetOutput(io.MultiWriter(os.Stderr, logbuf.Global()))
	server.SetChatLogOut(io.MultiWriter(os.Stdout, logbuf.Global()))

	// 管理端（默认 127.0.0.1:7864）：承载原 Python 控制台全部能力，供桌面 App 调用
	adminSrv := &admin.Server{Opt: admin.Options{
		BaseDir:     func() string { abs, _ := filepath.Abs("."); return abs }(),
		AuthDir:     cfg.AuthDir,
		DataDir:     filepath.Dir(cfg.StateFile),
		BackupDir:   filepath.Join(filepath.Dir(cfg.StateFile), "backups"),
		ConfigPath:  *cfgPath,
		DisplayBase: "http://localhost:7863/v1",
		GatewayBase: "http://127.0.0.1:7863",
		APIKey:      cfg.APIKey,
		Pool:        p,
		Up:          up,
		// 登录落盘 / 删除账号后热同步账号池（等价原控制台的 docker restart）
		OnAuthsChanged: func() {
			fresh, err := auth.LoadDir(cfg.AuthDir)
			if err != nil {
				log.Printf("admin: reload auths: %v", err)
				return
			}
			p.SyncToDir(fresh)
			log.Printf("admin: pool synced, %d account(s)", len(fresh))
		},
		// 配置保存后优雅退出，由桌面 App 的 sidecar 守护重拉生效
		OnConfigSaved: func() { stop() },
	}}
	go func() {
