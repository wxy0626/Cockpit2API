...
// chatLogOut 表格日志输出目标：默认"当前 os.Stdout"（每次写入时解引用，
// 兼容测试的 captureStdout 重定向）；main 可注入 MultiWriter
//（叠加 logbuf 环形缓冲，供管理端 /api/logs 读取）。
var chatLogOut io.Writer = stdoutWriter{}

// stdoutWriter 惰性解引用 os.Stdout：测试重定向后仍写往新管道。
type stdoutWriter struct{}

func (stdoutWriter) Write(p []byte) (int, error) { return os.Stdout.Write(p) }

// SetChatLogOut 注入表格日志输出目标（仅 main 启动期调用一次）。
func SetChatLogOut(w io.Writer) { chatLogOut = w }
...
func (s *chatStatsReader) parseSSELine(line string) {
	...
	if !s.seen {
		...
		// Windows 计时器粒度粗，极快路径可能测得 0；TTFB 语义上必为正，钳到最小可表示值
		if s.ttfb <= 0 {
